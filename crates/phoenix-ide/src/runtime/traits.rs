...
pub trait ToolExecutor: Send + Sync {
    ...
    /// Frozen model choices advertised by the conversation's `spawn_agents`
    /// schema. Spawn-time validation uses this same snapshot so descriptions
    /// and accepted IDs cannot drift if the live registry changes.
...
impl ToolExecutor for ToolRegistryExecutor {
    ...
    fn upgrade_to_work_mode(&self) {
        ...
        self.swap_registry(
            ToolRegistry::direct(self.agent_catalog.to_vec(), self.model_catalog.to_vec())
