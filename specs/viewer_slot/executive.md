...
| **REQ-VS-014:** Per-Conversation Viewer Persistence on In-App Nav | localStorage-backed last-viewer map (`phoenix:lastviewer:<slug>` → URL params snapshot) in `ui/src/storage/lastViewerStorage.ts`, covering all viewer kinds. `ViewerSlotProvider` writes on every non-empty slot, clears on explicit user close, and restores on in-app *entry* (a `scopeKey` change with `useLocation().key !== 'default'`) when the URL is bare. Restore is gated on conversation entry rather than any URL change, so a programmatic URL clear (normalization, browser falling-edge close) can't immediately re-restore the closed viewer. Cold reload deliberately does not restore (D1). Hard-delete cascade clears the entry via `useConversationsRefresh.ts` |
| **REQ-VS-015:** User Opens Message Viewer | `MessageContextMenu` emits the open-message command; `ConversationPage` opens `?viewer=message&message=<sequence_id>` and `MessageViewer` resolves the message markdown from the loaded conversation |
